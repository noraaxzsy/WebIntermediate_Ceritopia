...
import { subscribeToPushNotification, unsubscribeFromPushNotification } from "../data/api";
...
export async function subscribe() {
  ...
  try {
    ...
    const response = await subscribeToPushNotification({ endpoint, keys });
...
export async function unsubscribe() {
  ...
  try {
    ...
    const { endpoint, keys } = pushSubscription.toJSON();
    const response = await unsubscribeFromPushNotification(endpoint);
    ...
    if (!unsubscribed) {
      alert(failureUnsubscribeMessage);
      await subscribeToPushNotification({ endpoint, keys });
